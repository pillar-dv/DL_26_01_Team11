...
def simulate_pdf_generation(pages_num, target_energy='태양광 (Solar)', sel_region='경기도'):
    # 가짜 예측 데이터 준비
    ...
    # 임시 차트 이미지 경로 설정 및 실제 더미 이미지 생성
    ...
    if pages_num == 2:
        ...
        pdf.cell(0, 5.2, f'  2. 적용 예측 모델 : LSTM 독립 적합 신경망 모델', border=0, ln=1)
        pdf.ln(3)
        pdf.set_font('Malgun', 'B', 11)
        ...
        pdf.cell(0, 5.2, f'  2. 예측 평균 기온 : {avg_temp:.1f} °C  /  평균 일사량 : {avg_wind_or_insol:.2f} MJ/m2', border=0, ln=1)
    elif pages_num == 3:
        pdf.set_font('Malgun', 'B', 11)
        pdf.cell(0, 8, 'Ⅱ. 예보 일자 및 타겟 사양 명세', border=0, ln=1)
        pdf.set_font('Malgun', '', 9)
    ...
    if pages_num == 2:
        ...
        pdf.set_font('Malgun', '', 9)
        cor_detail = '  1. 물리 기상-발전 상관관계 해석 :\n    가. 기온 상승 시 모듈 온도 저하로 발전 효율이 미세 하강하는 반비례 상관성이 나타남.\n    나. 습도/미세먼지가 일사량을 약 10~15% 수준 감쇄시킴.'
        ...
        pdf.set_font('Malgun', '', 9)
        pdf.cell(0, 5.2, f'  1. 적용 대상 일자 : {formatted_target_date}  /  분석 타겟 지역 : {sel_region} 전역', border=0, ln=1)
        pdf.cell(0, 5.2, f'  2. 적용 예측 모델 : LSTM 지자체별 독립 적합 신경망 모델', border=0, ln=1)
        pdf.ln(2)
        pdf.set_font('Malgun', 'B', 11)
