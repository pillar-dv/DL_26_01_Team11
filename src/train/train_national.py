...
wind_mapping = {
    # 강원도: 태백산맥 능선 집중 (대관령, 태백, 정선, 삼척, 평창)
    '강원도': {'weather': ['대관령', '태백'], 'dust': ['대관령', '태백']},
    # 제주도: 제주 전역 (고산=서부, 성산=동부)
    '제주도': {'weather': ['고산', '성산'], 'dust': ['고산', '성산']},
    # 경상북도: 포항 해안 + 영양·울진·청송·군위 내륙 (태백산맥 인접)
    '경상북도': {'weather': ['포항', '태백'], 'dust': ['포항', '태백']},
    # 전라북도: 군산 해상 + 부안 위도 해상 중심
    '전라북도': {'weather': ['군산', '부안'], 'dust': ['군산', '부안']},
    # 육지 통합: 강원(대관령·태백) + 경북(포항) + 전북(군산) + 전남(목포) 커버
    '육지': {'weather': ['대관령', '태백', '포항', '군산', '목포'], 'dust': ['대관령', '태백', '포항', '군산', '목포']}
}
