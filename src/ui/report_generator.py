            ...
            with st.spinner('AI 시뮬레이션 및 행정 문서 서식 템플릿 컴파일 중...'):
                ...
                if target_energy == '태양광 (Solar)':
                    ...
                    if not hist_df.empty:
                        ...
                        base_profile = solar_df[solar_df['지역'] == sel_region].groupby('시간')[agg_feats].mean().reset_index()
                else:
                    ...
                    if not hist_df.empty:
                        ...
                        base_profile = wind_df[wind_df['지역'] == sel_region].groupby('시간')[agg_feats].mean().reset_index()
                if use_stochastic:
                ...
                sim_weather_df = sim_weather_df.sort_values('시간').reset_index(drop=True)
                ...
                if target_energy == '태양광 (Solar)':
                    ...
                    if m_solar is not None:
                        for t in range(24):
                            ...
                            window = sim_df_extended.iloc[t+1 : t+25].copy()
                            ...
                            # [FIX] 물리 가드: window 끝점(t+24 = hour t)의 기상
